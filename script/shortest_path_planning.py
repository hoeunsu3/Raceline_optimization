...
def main():
    # ==================== Read Track Data ====================
    data = np.loadtxt('./paths/Silverstone.csv', skiprows=1, delimiter=',')
    ...
    for i in range(n - 1):
        H[i,i]      = H[i,i]     + normal_x[i]**2               + normal_y[i]**2
        H[i+1,i]    = H[i+1,i]   - normal_x[i]*normal_x[i+1]    - normal_y[i]*normal_y[i+1]
        H[i,i+1]    = H[i,i+1]   - normal_x[i]*normal_x[i+1]    - normal_y[i]*normal_y[i+1]
        H[i+1,i+1]  = H[i+1,i+1] + normal_x[i+1]**2             + normal_y[i+1]**2

    H = 2*H

    for i in range(n - 1):
        B[i]    = B[i]      - 2*(inner[i+1,0] - inner[i,0])*normal_x[i]     - 2*(inner[i+1,1] - inner[i,1])*normal_y[i]
        B[i+1]  = B[i+1]    + 2*(inner[i+1,0] - inner[i,0])*normal_x[i+1]   + 2*(inner[i+1,1] - inner[i,1])*normal_y[i+1]
    ...
    res = S(h=H, g=B.T, a=Aeq, lba=beq, uba=beq, lbx=lb, ubx=ub)
    alpha_opt = res['x']
    ...
    for i in range(n-1):
        ...
        SP[i,0] = inner[i,0] + alpha_opt[i] * (twr_interp[i] + twl_interp[i]) * np.sin(-psi)
        SP[i,1] = inner[i,1] + alpha_opt[i] * (twr_interp[i] + twl_interp[i]) * np.cos(-psi)

    plt.plot(inner[:,0], inner[:,1], label='inner')
    plt.plot(outer[:,0], outer[:,1], label='outer')
    plt.plot(SP[:,0], SP[:,1], '--', label='SP')
    plt.legend()
    plt.show()
